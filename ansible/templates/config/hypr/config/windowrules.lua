...
hl.window_rule({
	match = { float = true },
	move = {
		"max(min(cursor_x - 5), monitor_w - window_w), 0)",
		"max(min(cursor_y - 2), monitor_h - window_h), 0)",
	},
})
...
-- ╰──────────────╯
-- Spotify ────────────────────────────────────────────────────────
hl.window_rule({ match = { class = "^(.*[Ss]potify)$" }, workspace = mediaWorkspace })

-- Spotify minibrowser ────────────────────────────────────────────
hl.window_rule({
	match = { float = true, class = "^(.*[Cc]hromium-browser)" },
	move = {
		"(monitor_w - 5) - " .. MINIPLAYER_W,
		45,
	},
	size = { MINIPLAYER_W, MINIPLAYER_H },
})
hl.window_rule({ match = { class = "^(.*[Vv]lc)$" }, workspace = mediaWorkspace })
